...
pip install youtube-transcript-api  # 字幕提取
...
# 字幕提取（无需 API Key）
python scripts/analyze_youtube.py "YOUTUBE_URL" --subtitles
```
...
- `--subtitles` - 提取原始字幕，而非 Gemini 分析
- `--lang LANGS` - 字幕语言优先级，逗号分隔（默认：zh-Hans,en）
...
# 提取字幕（文本格式）
python scripts/analyze_youtube.py "URL" --subtitles

# 提取字幕（JSON 格式）
python scripts/analyze_youtube.py "URL" --subtitles --format json

# 指定字幕语言
python scripts/analyze_youtube.py "URL" --subtitles --lang "en,ja"
```
