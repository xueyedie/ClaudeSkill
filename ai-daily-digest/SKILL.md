...
### "Gemini 模型不存在 / 模型不可用"
Gemini 的模型 ID 会随时间变更或下线。脚本在检测到“当前模型不可用/不存在”时，会自动：
1. 使用你的 `GEMINI_API_KEY` 调用 `v1beta/models` 列出**当前 Key 可用**且支持 `generateContent` 的模型
2. 在终端中打印模型列表
3. 让你输入序号或模型名进行选择，然后自动用你选的模型继续运行

如果你希望固定默认模型，可手动设置：
- 环境变量：`GEMINI_MODEL`
- 命令行参数：`--gemini-model <model>`

**手动自检（PowerShell）**：列出你账号当前可用的模型（最权威）
