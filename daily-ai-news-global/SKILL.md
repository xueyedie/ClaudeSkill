...
## 阶段 0.6：来源访问方式缓存（2026-03-24 本机已验证）
...
| `opencli twitter profile <username> -f json` | `opencli` | 成功 | `xai` / `elonmusk` / `karpathy` 已验证 |
| `opencli twitter search "<query>" -f json` | 托管浏览器 | 失败 | 当前报 `SPA navigation to /search failed`，不要反复重试 |
| `https://x.ai/` | 托管浏览器 | `web_fetch`/HTTP 失败 | 403 Cloudflare challenge，直接走托管浏览器 |
...
4. **X/Twitter 例外**：当前机器上 `twitter profile` 可用，但 `twitter search` 已知不可用。  
   - 账号元信息：优先 `opencli twitter profile`
   - 最近帖子 / 关键词搜索 / 白名单账号最新动态：直接托管浏览器
...
- 对于已在“阶段 0.6”验证过的来源，优先沿用已验证方式：
  ...
  - `x.ai` / `tesla.com/AI` / `tesla.com/blog`：直接托管浏览器
  - `X/Twitter` 关键词搜索：当前机器不要再试 `opencli twitter search`，直接托管浏览器
...
- 当前机器的白名单抓取路由默认按“阶段 0.6 来源访问方式缓存”执行：
  ...
  - 若要看最近帖子、搜索关键词或确认最新公开动态：直接托管浏览器，不要再试 `opencli twitter search`
  - `x.ai`：直接托管浏览器，不要再试 `web_fetch`
