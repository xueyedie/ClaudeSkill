...
- **搜索查询**：使用 WebFetch 访问搜索引擎（如 `https://www.google.com/search?q=...`）执行搜索查询，解析返回的搜索结果页面提取链接和摘要。也可以使用托管浏览器（browser）打开搜索引擎页面执行搜索，适用于搜索结果被反爬限制的场景。
- **页面内容抓取**：优先使用 WebFetch 直接抓取目标 URL 的页面内容。
- **需要登录态的页面**（如 B站关注列表、小红书等）：使用托管浏览器（browser）访问，浏览器已配置持久化 profile，可复用登录态。
- **降级策略**：WebFetch 失败时，尝试用托管浏览器访问同一 URL；浏览器也失败则跳过该来源。
